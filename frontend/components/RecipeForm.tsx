...
export function RecipeForm({ initialValue, submitLabel, onSubmit }: Props) {
  ...
  return <form onSubmit={submit} className="recipe-form">
    <section className="content-card form-card"><div className="section-heading"><div><p className="eyebrow">The basics</p><h2>Recipe details</h2></div></div><label>Recipe name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Lemon herb chicken" required /></label></section>
    <div className="form-section">
      <div className="section-heading"><div><p className="eyebrow">Build your list</p><h2>Ingredients</h2></div><button type="button" className="button secondary" onClick={() => setIngredients((current) => [...current, blankIngredient()])}>+ Add ingredient</button></div>
      <div className="ingredient-table"><div className="ingredient-table-header" aria-hidden="true"><span>Name</span><span>Quantity</span><span>Unit</span><span>Action</span></div>{ingredients.map((ingredient, index) => <div className="ingredient-row" key={index}>
        ...
        <button type="button" className="link-button" aria-label={`Remove ingredient ${index + 1}`} disabled={ingredients.length === 1} onClick={() => setIngredients((current) => current.filter((_, itemIndex) => itemIndex !== index))}>Remove</button>
      </div>)}</div>
    </div>
